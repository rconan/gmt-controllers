...
mod tests {
    ...
    fn impulse() {
        ...
        let pzt_7_Cfb_imp_t: Vec<f64> = mat.var("pzt_7_Cfb_imp_t").unwrap();
        let pzt_7_Cfb_imp_y: Vec<f64> = mat.var("pzt_7_Cfb_imp_y").unwrap();

        let n = pzt_7_Cfb_imp_t.len();

        let sim_y: Vec<_> = pzt_7_Cfb_imp_y.chunks(n).take(1).flatten().collect();
        dbg!(&sim_y[..10]);

        let mut ctrl = FsmPiezo7::new();
        let mut y = vec![];
        for i in 0..n {
            ctrl.inputs
                .pzt_error
